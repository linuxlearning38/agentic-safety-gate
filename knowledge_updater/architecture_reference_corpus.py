...
ARCHITECTURE_REFERENCE_DOCS = [
    {
        ...
        "architecture_notes": "Use Kafka when a system needs a durable event backbone between producers and consumers. In architecture explanations, describe Kafka as the event transport and fan-out layer rather than as a database. Pair it with stream processors for aggregation and with serving stores for queryable views.",
    ...
    },
    {
        ...
        "architecture_notes": "When AVA explains architectures that include Kafka, it should call out producers, topics, partitions, consumers, and replayability. Kafka is strongest as a shared event log connecting loosely coupled services.",
    ...
    },
    {
        ...
        "architecture_notes": "Use this as the grounding model for general event-driven explanations. Distinguish publish-subscribe from event streaming: pub-sub focuses on delivery to subscribers, while streaming uses a durable log that consumers can replay from arbitrary positions.",
    ...
    },
    {
        ...
        "architecture_notes": "Use this as a style-selection anchor. When AVA is asked why event-driven systems are used, it should mention loose coupling, elasticity, and the ability for multiple consumers to react independently to the same event stream.",
    ...
    },
    {
        ...
        "architecture_notes": "When AVA explains event-driven designs, it should avoid treating all event systems as identical. It should identify whether the events are simply notifications, durable state changes, or the source of truth for derived models.",
    ...
    },
    {
        ...
        "architecture_notes": "Use this to explain caches like Redis or EVCache. AVA should describe the cache as a latency and load-reduction layer in front of the durable store, not as the long-term system of record.",
    ...
    },
    {
        ...
        "architecture_notes": "When AVA explains caching layers in large systems, it should mention that cache efficiency depends on access patterns and freshness requirements. Caches are most useful for repeated, hot reads that would otherwise burden the backing store.",
    ...
    },
    {
        ...
        "architecture_notes": "AVA should use this as a reminder to justify why a cache exists: hot reads, low latency, or protection of the primary store. It should avoid describing every cache as interchangeable with a durable database.",
    ...
    },
    {
        ...
        "architecture_notes": "When AVA explains architectures containing Zuul, it should identify Zuul as the edge gateway in the synchronous request path. Zuul is not the database or event backbone; it is the front-door routing and control layer.",
    ...
    },
    {
        ...
        "architecture_notes": "Use this to ground explanations that ask why Zuul exists in a large-scale architecture. AVA should mention routing, filtering, resiliency, and edge traffic control instead of reducing Zuul to a generic reverse proxy.",
    ...
    },
    {
        ...
        "architecture_notes": "When AVA explains EVCache, it should identify it as a replicated cache for hot reads, not as the system of record. A strong explanation calls out local-zone reads for latency and multi-zone replication for availability.",
    ...
    },
    {
        ...
        "architecture_notes": "When AVA explains Mantis in a system architecture, it should position Mantis in the observability or streaming analytics path rather than in the core request-serving path. Mantis is a stream-processing/runtime platform, not the user-facing gateway.",
    ...
    },
    {
        ...
        "architecture_notes": "AVA should describe Samza as the stream-processing layer that consumes from the event backbone, performs stateful processing or aggregation, and emits derived outputs. Samza should not be described as the primary transport or serving database.",
    ...
    },
    {
        ...
        "architecture_notes": "When AVA explains architectures with Cassandra, it should position Cassandra as the durable, distributed state store optimized for availability and scale. Cassandra is usually part of the data-serving layer, often paired with caches for hot reads.",
