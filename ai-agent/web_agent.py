...
def get_context(question):
    ...
    results = collection.query(query_embeddings=[q_embed["embedding"]], n_results=4)
...
def ask(question):
    ...
    system_prompt = (
        "You are a strict DevOps and Infrastructure assistant.\n\n""You ONLY answer questions related to:\n"
        "- Server disk space, memory, CPU, system resources\n""- Kubernetes, Docker, Linux system administration\n""- Terraform, OPA, infrastructure security\n""- CI/CD pipelines, DevOps automation\n""- Shell commands and system operations\n""- The policy knowledge base and infrastructure documents\n\n""If the user asks ANYTHING unrelated (medical, personal, financial, general knowledge)\n""respond with exactly: This assistant only answers DevOps and infrastructure questions.\n\n"
        "POLICY CONTEXT (use this to answer policy questions directly):\n" + context + "\n\n"
        "RULES:\n"
    ...
    if "COMMAND:" in reply:
        for line in reply.split("\n"):
            if line.startswith("COMMAND:"):
                ...
                followup = ollama.chat(model="llama3.1:8b", messages=followup_messages)
