...
def main() -> int:
    ...
    failures.extend(
        [
            ...
            check("network readiness flushes resolver cache", "resolvectl flush-caches" in _step("network_ready").command),
            check("network readiness restarts resolver on repeated misses", "restart systemd-resolved" in _step("network_ready").command),
            check("apt update has extended retry loop", "for i in 1 2 3 4 5 6" in _step("package_update").command),
            check("apt install has extended retry loop", "for i in 1 2 3 4 5" in _step("install_web_packages").command),
